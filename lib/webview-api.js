...
module.exports = function buildAPI (browserWindow, panel, webview) {
  ...
  webContents.executeJavaScript = webview.stringByEvaluatingJavaScriptFromString
  ...
  if (!FrameLoadDelegateClass) {
    FrameLoadDelegateClass = ObjCClass({
      ...
      state: NSDictionary.dictionaryWithDictionary({
        lastQueryId: null,
        wasReady: 0
      }),
      utils: null,
      ...
      'webView:didChangeLocationWithinPageForFrame:': function (
        ...
        if (
          ...
          this.state.lastQueryId = query.actionId
          try {
            query.pluginArgs = JSON.parse(query.pluginArgs)
          } catch (err) {}
          this.utils.emitEvent(query.pluginAction, query.pluginArgs)
        }
      ...
      'webView:didFailLoadWithError:forFrame:': function (
        ...
        this.utils.emitEvent('did-fail-load', [error])
      },
      ...
      'webView:didFinishLoadForFrame:': function (webView, webFrame) {
        ...
        this.utils.emitEvent('did-finish-load')
        this.utils.emitEvent('did-frame-finish-load')
        this.utils.emitEvent('dom-ready')
      },
      ...
      //   webFrame
      // ) {
      //   this.utils.emitEvent('did-get-redirect-request')
      // },
      ...
      //   webFrame
      // ) {
      //   this.utils.emitEvent('will-navigate', [{}, String(url)])
      // }
    })
  }
  ...
  frameLoadDelegate.utils = NSDictionary.dictionaryWithDictionary({
    ...
    emitBrowserEvent: browserWindow.emitEvent.bind(browserWindow),
    emitEvent: webContents.emitEvent.bind(webContents),
